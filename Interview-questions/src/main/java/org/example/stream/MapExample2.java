...
public class MapExample2 {
    public static void main(String[] args) throws IOException, URISyntaxException {
       ...
       List<String> words1 =
        ...
        employeeList.stream()
                .collect(Collectors.groupingBy(Employee::getDepartment,
                        Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparingLong(Employee::getSalary)),
                                // Get the max salaried employee for each department
                                Optional::get)));

        Map<String, Employee> maxSalaryByDepartment = employeeList.stream()
                        .collect(Collectors.groupingBy(Employee::getDepartment,
        ...
        //Find department with highest number of employee and there count
        Optional<Map.Entry<String, Long>> maxEntry = empCountDeptWise.entrySet().stream().max(Map.Entry.comparingByValue());
